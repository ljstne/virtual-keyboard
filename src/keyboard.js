...
const tieMouseEvents = () => {
  ...
  keyboardContainer.addEventListener('mouseup', (event) => {
    ...
    if ((pressedKeys.CapsLock === true || pressedKeys.Shift === true) && event.target.getAttribute('data-key') === 'ShiftLeft') {
      keyboard.fillKeys(true, language);
    } else if (pressedKeys.CapsLock === undefined && pressedKeys.Shift === undefined && event.target.getAttribute('data-key') === 'ShiftLeft') {
      keyboard.fillKeys(false, language);
    }
